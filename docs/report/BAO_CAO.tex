...
Detector (3-stage ML pipeline) \textrightarrow{} Prometheus (time-series storage) \textrightarrow{} Grafana (dashboard). Dữ liệu đầu vào là flow đã ghi sẵn, được replay theo lô.

\subsubsection{Các thành phần hệ thống}
\begin{itemize}
    \item \textbf{Detector (host)}: Đọc flow files, chạy 3 stage, export metrics.
    \item \textbf{Prometheus (container)}: Thu thập/lưu trữ metrics tại \texttt{http://localhost:9090}.
    \item \textbf{Grafana (container)}: Truy vấn Prometheus, hiển thị dashboard tại \texttt{http://localhost:3000}.
    \item \textbf{Victim (container)}: Mô phỏng máy đích bị tấn công (phục vụ demo).
    \item \textbf{Attacker (container)}: Mô phỏng máy tấn công (phục vụ demo).
\end{itemize}

\subsubsection{Luồng dữ liệu}
\begin{itemize}
    \item Input: \texttt{flows\_c.txt}, \texttt{flows\_s.txt} (Argus flows).
    \item Xử lý: Detector đọc batch, suy luận 3 stage, đẩy metrics sang Prometheus.
    \item Hiển thị: Grafana truy vấn PromQL và hiển thị kết quả batch gần nhất.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Tổng quan dashboard}
    \label{fig:ddos_dashboard_overview}
\end{figure}

Nhận xét: Dashboard cung cấp 3 layers thông tin:
\begin{itemize}
    \item \textbf{Overview layer} (Stat panels): Traffic Classification Results, Attack Detection Rate
    \item \textbf{Distribution layer} (Charts): Attack Type Distribution, DDoS Variants Distribution, Attack Classification Breakdown
    \item \textbf{Technical layer} (Gauges): Source IP Entropy, Packet Rate per IP
\end{itemize}
Bố cục này cho phép operator đánh giá nhanh từ tổng quan đến chi tiết sau mỗi lần replay batch.

\subsubsection{Traffic Classification Results (Stat)}
\begin{verbatim}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{grafana_sc/Traffic_Classification_Results.png}
    \caption{Traffic Classification Results}
    \label{fig:traffic_classification_results}
\end{figure}
Nhận xét: Panel này cung cấp cái nhìn tổng quan nhanh chóng về phân bố traffic. Từ ảnh có thể thấy:
\begin{itemize}
    \item DoS chiếm tỷ lệ cao nhất (17,851 windows - màu orange)
    \item DDoS: 697 windows (màu đỏ)
    \item Reconnaissance: 270 windows (màu vàng)
    \item Normal: 627 windows (màu xanh)
\end{itemize}
...
\subsubsection{Attack Type Distribution (Bar Chart, Stage 2)}
\begin{verbatim}
sum by (attack_type) (ddos_attack_windows_total)
\end{verbatim}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{grafana_sc/Attack_Type_Distribution_(Stage2).png}
    \caption{Attack Type Distribution (Stage 2)}
...
\begin{itemize}
    \item DoS (màu vàng): áp đảo tập dữ liệu
    \item DDoS (màu đỏ): tỷ lệ thấp hơn DoS nhiều lần
    \item Reconnaissance: tỷ lệ nhỏ nhất
\end{itemize}
...
\subsubsection{DDoS Variants Distribution (Donut, Stage 3)}
\begin{verbatim}
sum by (ddos_variant) (ddos_attack_windows_total{attack_type="ddos"})
\end{verbatim}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{grafana_sc/DDoS_Variants_Distribution_(Stage3).png}
    \caption{DDoS Variants Distribution (Stage 3)}
    \label{fig:ddos_variants_distribution}
\end{figure}
Nhận xét: Donut chart thể hiện phân bố các biến thể DDoS từ Stage 3. Từ ảnh có thể thấy:
\begin{itemize}
    \item TCP: ~32.1\% (tỷ lệ cao nhất)
    \item Normal (unclassified): ~30.4\% (các flow không phân loại được)
    \item UDP: ~19.1\%
    \item HTTP: ~18.4\%
\end{itemize}
Tỷ lệ Normal cao là do model Stage 3 được train trên Bot-IoT CSV (có port info rõ ràng) nhưng inference trên Argus aggregated flows (port info bị mở). Donut chart visualization giúp dễ dàng so sánh tỷ lệ giữa các variants.

\subsubsection{Packet Rate per Destination IP (Bar Gauge)}
\begin{verbatim}
avg(ddos_packet_rate) by (dst_ip) > 0.5
\end{verbatim}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{grafana_sc/Packet_Rate_per_Destination_IP.png}
    \caption{Packet Rate per Destination IP}
    \label{fig:packet_rate_per_dst}
\end{figure}
...
\begin{itemize}
    \item Green (<0.5 pps): Normal traffic
    \item Yellow (0.5--2 pps): DoS attack
    \item Orange (2--10 pps): DDoS medium intensity
    \item Red (>10 pps): DDoS high intensity
\end{itemize}
...
\subsubsection{Source IP Entropy (Gauge)}
\begin{verbatim}
topk(15, max(ddos_src_entropy) by (dst_ip) > 1)
\end{verbatim}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{grafana_sc/Source_IP_Entropy.png}
    \caption{Source IP Entropy}
    \label{fig:source_ip_entropy}
\end{figure}
...
\begin{itemize}
    \item Green (<1.5): DoS/Normal - ít nguồn tấn công
    \item Yellow (1.5--3): DDoS medium scale - độ phân tán trung bình
    \item Red (>3): DDoS high scale - nhiều nguồn phân tán
\end{itemize}
...
\subsubsection{Attack Classification Breakdown (Pie)}
\begin{verbatim}
sum(ddos_normal_windows_total)
sum(ddos_attack_windows_total{attack_type="dos"})
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{grafana_sc/Attack_Classification_Breakdown.png}
    \caption{Attack Classification Breakdown}
    \label{fig:attack_classification_breakdown}
\end{figure}
Nhận xét: Pie chart này cung cấp phân tích chi tiết nhất với 7 categories, kết hợp cả 3 stages. Từ ảnh có thể thấy:
\begin{itemize}
    \item DoS (màu xám): chiếm phần lớn diện tích (~93\%)
    \item DDoS HTTP (màu đỏ): khoảng 18.4\%
    \item DDoS TCP (màu light-blue): khoảng 32.1\%
    \item DDoS UDP (màu vàng): khoảng 19.1\%
    \item DDoS Normal (màu dark-gray): khoảng 30.4\% (unclassified)
    \item Reconnaissance (màu light-orange): khoảng 1.4\%
    \item Normal (màu light-purple): khoảng 3.3\%
\end{itemize}
...
\subsubsection{Attack Detection Rate (Stat, Stage 1)}
\begin{verbatim}
sum(ddos_attack_windows_total)
\end{verbatim}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{grafana_sc/Attack_Detection_Rate_(Stage1).png}
    \caption{Attack Detection Rate (Stage 1)}
    \label{fig:attack_detection_rate}
\end{figure}
...
\textbf{DDoS Variants Classification} đạt Accuracy \textbf{99.92\%} trên bài toán phân loại chi tiết các biến thể DDoS:
\begin{itemize}
    \item Phân loại chính xác HTTP, TCP, UDP với F1-score gần 100\%
    \item Training time < 2 phút trên CPU, phù hợp resource-constrained environments
\end{itemize}
